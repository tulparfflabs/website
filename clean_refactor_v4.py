...
for f in files:
    ...
    # 1. Update the Unified CSS block
    ...
    # 2. Make absolutely sure the HTML hamburger is correct
    # (If the user reverted their file, this will fix it again)
    if 'class="hamburger-menu"' in content:
        content = re.sub(
    ...
    elif 'id="hamburger"' in content and 'class="hamburger"' not in content:
        content = re.sub(
    ...
    # Check if the file has NO hamburger at all (user might have deleted it in frustration)
    if 'id="hamburger"' not in content and 'class="hamburger"' not in content:
        # Insert it right after the nav-logo
        content = re.sub(
            r'(<div class="nav-logo">[\s\S]*?</div>|<a href="[^"]*" class="nav-logo"[^>]*>[\s\S]*?</a>)',
            r'\1\n    <div class="hamburger" id="hamburger" onclick="document.getElementById(\'navbar\').classList.toggle(\'open\')">\n        <span></span>\n        <span></span>\n        <span></span>\n    </div>',
            content
        )
